...
case "${1:-help}" in
  ...
  update)
    # Re-run installer
    curl -fsSL "https://raw.githubusercontent.com/Eryooo/designos/main/install.sh" | bash
    ;;
  list)
    echo "已安装的 Skills:"
  ...
  help|--help|-h|"")
    ...
    echo "  designos update   升级到最新版本"
    echo "  designos inject   兜底：当 IDE 不识别 /uxeval 时，注入 AGENTS.md 到当前目录"
...
if [ "$INSTALL_MODE" = "local" ]; then
  ...
  echo ""
  printf "  ${BOLD}建议${NC} → 退出 IDE，在 macOS ${BOLD}终端 App${NC} 里重新运行此脚本：\n"
  printf "         ${BLUE}curl -fsSL https://raw.githubusercontent.com/$REPO/$BRANCH/install.sh | bash${NC}\n"
  echo ""
