...
**执行方式**：
- 自主探测当前环境的可用工具和能力
- 如果有缺失 → 在开始评估前一次性告知用户需要安装什么，给出适合当前环境的安装命令
- 用户确认安装完成（或选择接受降级）后再进入 Step 1
...
✅ 截图读取：多模态视觉能力可用
✅ 文件写入：正常
⚠️ Excel 生成：openpyxl 未安装（可选，不影响主流程）
   → 如需生成 Excel 报告，请执行：pip install openpyxl

准备就绪，开始评估。
```

**如果有关键依赖缺失**（如 PDF 解析不可用）：
```
环境预检发现问题：
...
⚠️ 不安装的风险：只能用多模态"看"PDF，可能漏读复杂内容，影响评估质量。
```

不要跳过预检直接开始。预检通过后再进入 Step 1。
