...
class MCPClient(IMCPClient):
    ...
    async def call_tool(
        ...
        transport: StdioTransport | SSETransport = self._transport_for(config)
        _log.info("mcp.call", server=server, tool=tool)
        return await transport.call(tool, args)

    def _transport_for(self, config: MCPServerConfig) -> StdioTransport | SSETransport:
        if config.name in self._transports:
            return self._transports[config.name]
        if config.transport is MCPTransport.STDIO:
            transport: StdioTransport | SSETransport = StdioTransport(config)
